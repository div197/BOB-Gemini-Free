...
| The native desktop app exposes an explicit update check | VERIFIED_BY_UNIT_TEST | `internal/updater/desktop.go` selects only official platform package names; `internal/updater/updater_test.go` covers branded and legacy migration names | The check uses the stable `/releases/latest` endpoint; the prerelease is deliberately manual and the menu does not silently install or replace the app. |
