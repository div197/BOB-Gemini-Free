...
| A public native desktop preview is available for controlled evaluation | VERIFIED_LIVE | Manually published prerelease `v0.1.7-preview.3` contains branded macOS universal `.dmg`/`.zip`, Windows x64 `.exe`, `RELEASE-NOTICE.txt`, and `SHA256SUMS` assets | This is a beta preview only: no Apple notarization, Windows publisher signature, Linux asset, clean-device matrix, signed manifest, automatic native updater, or production student-readiness claim. |
