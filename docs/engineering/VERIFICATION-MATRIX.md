...
| The current source Preview 8 candidate is locally packaged and signed without being represented as public | VERIFIED_LOCAL | Reviewed runtime source commit `7395243c90eec3f884dd49067b64dd549aab1956` passed the clean-source gate, Wails universal build, Keychain-backed manifest signer, exact-asset verifier, DMG-layout check, packaged `/healthz` and static-route smoke, updater transition tests, browser settings smoke, and current source tests; later main commits only reconcile the receipt; receipt: `PREVIEW-8-CANDIDATE-VERIFICATION-2026-08-31.md` | `v0.2.0-preview.8` is not published or downloadable; installed Preview 6 → Preview 8 replacement, public-byte reconciliation, rollback, clean-device, Apple trust, provider, and pilot gates remain open. |
