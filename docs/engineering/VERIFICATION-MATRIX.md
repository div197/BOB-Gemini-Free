...
| The native update-check dialog exposes raw transport or GitHub metadata details, or makes a read-only failure look like an app change | VERIFIED_BY_UNIT_TEST | `cmd/desktop/updates.go` maps cancellation, timeout, network, and metadata failures to bounded actionable messages that state no app change occurred; `cmd/desktop/main_test.go` rejects timeout, HTTP status, and low-level transport detail leakage | Native dialog rendering and the actual GitHub network remain device/external evidence surfaces. |
