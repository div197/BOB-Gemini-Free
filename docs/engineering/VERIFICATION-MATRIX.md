...
| An arbitrary web origin can cause privileged no-key gateway requests | VERIFIED_BY_INTEGRATION_TEST | Baseline wildcard behavior was reproduced before the fix; the current security tests reject an untrusted origin, a different loopback port, and a non-loopback Host/origin pair, while allowing only the exact literal-loopback gateway origin or an explicit configured origin | Browser exploitability still depends on browser/PNA behavior; a real browser-origin exploit test and capability-token design remain release gates. |
...
| Updater replacement is atomic and rollback-safe on all platforms | VERIFIED_BY_UNIT_TEST | Unix uses same-filesystem candidate/target renames; the desktop helper flushes Unix directory entries after backup, activation, and rollback transitions; mocked tests isolate the candidate and verify bytes/checksum before replacement, while `desktop_durability_test.go` injects activation-sync failure and proves restoration | Windows retains a platform-specific rollback/copy path without a portable directory-fsync contract; recursive app-bundle durability and a real installed executable were deliberately not claimed or touched. |
...
| Release source provenance | `scripts/verify-release-source.sh` requires a Git worktree with no modified/untracked paths, validates the stable/preview version against the `Makefile` base, compares the generated Web bundle without rewriting it, and checks the canonical Ed25519 key block against Makefile/package inputs, both standalone installers, Docker metadata, and preview packager defaults | VERIFIED_IN_SOURCE | This protects local packaging entry points and key/version coherence; private-key matching, commit review, tag publication, and final public-asset reconciliation remain operator gates. |
...
worktree. The public release remains unchanged.
...
| The macOS v0.2.0 candidate is package-valid | VERIFIED_BY_INTEGRATION_TEST | Fresh `make desktop-preview-mac` on clean commit `cec4c8e` produced a Wails universal app, ZIP, DMG, Applications shortcut, ad-hoc code signature, SHA-256 checks, bundle metadata, local PWA routes, and native GUI quit/shutdown proof; the local Preview 2 release directory was then signed and verified through the Keychain-backed Ed25519 flow, and the exact app passed a real launch, `/healthz`, root-route, and graceful-shutdown check; the public `v0.2.0-preview.1` bridge was separately re-downloaded with checksum/signature verification | `spctl` rejection is expected without Apple notarization; local package verification proves neither a public upload nor clean-device update or student rollout. |
