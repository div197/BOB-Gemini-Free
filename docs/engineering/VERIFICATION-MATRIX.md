...
| Dynamic `at`/build tokens are extracted from Gemini HTML | VERIFIED_BY_UNIT_TEST | `internal/gemini/auth.go:19-22,118-151`; `internal/multimodal/tokens.go` bounds page responses/token values, refuses redirects/non-success responses, and preserves the previous set after failed refresh; `internal/multimodal/multimodal_test.go` covers valid refresh, oversized response, and redirect behavior | Regex coverage is still narrow and provider token semantics remain upstream-dependent. |
...
| The release source gate rejects updater-key and version/channel drift before packaging | VERIFIED_BY_INTEGRATION_TEST | `scripts/verify-release-source.sh` passed for stable and preview fixtures and failed closed when a standalone installer key was changed; PR [#33](https://github.com/div197/BOB-Gemini-Free/pull/33) merged this guard on `main` | This validates source/package inputs only; it does not sign, upload, or verify the public release assets. |
...
| UTF-8 error truncation corrupts a provider/API error or violates its output bound | VERIFIED_BY_UNIT_TEST | `sanitizeMessage` truncates by rune after removing credential/URL-like content; `internal/geminiapi/geminiapi_test.go` covers 500 Devanagari runes | Error text is intentionally shortened and is not a diagnostic transcript. |
