...
| A public native desktop preview is available for controlled evaluation | VERIFIED_LIVE | Manually published prerelease `v0.1.7-preview.5` contains the corrected branded macOS universal `.dmg`/`.zip`, `RELEASE-NOTICE.txt`, `SHA256SUMS`, and detached `SHA256SUMS.sig` assets | This is a beta preview only: no Apple notarization, Windows publisher signature for Preview 5, Linux asset, clean-device matrix, silent updater, or production student-readiness claim. |
...
| The native desktop app exposes an explicit update check | VERIFIED_BY_UNIT_TEST | `internal/updater/desktop.go` selects only official stable or `preview.N` channels; `internal/updater/updater_test.go` covers branded/legacy names, prerelease ordering, and signed-manifest discovery | Preview 5 can offer a consented verified update; it does not silently install, remove the macOS warning, or replace platform publisher trust. |
