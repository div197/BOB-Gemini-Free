...
| The Wails desktop process honors a user's local config/cookie without widening its network boundary | VERIFIED_BY_UNIT_TEST | `cmd/desktop/config_test.go` verifies cookie discovery while forcing loopback, empty API keys, and empty remote origins | First-run Wails login UX is not implemented; authenticated features remain per-user/session-dependent. |
| The public release is a student-ready cross-platform Wails installer set | UNKNOWN | GitHub release inspection found CLI assets in `v0.1.5` but no Wails desktop installers; only macOS ARM64 ad-hoc native smoke testing is complete | Requires native Windows/Linux builds, clean-device tests, platform signing, macOS notarization, and manual release publication. |
| A public native desktop preview is available for controlled evaluation | VERIFIED_LIVE | Manually published prerelease `v0.1.7-preview.2` contains the corrected macOS universal `.dmg`/`.zip`, Windows x64 `.exe`, `RELEASE-NOTICE.txt`, and `SHA256SUMS` assets | This is a beta preview only: no Apple notarization, Windows publisher signature, Linux asset, clean-device matrix, signed manifest, automatic native updater, or production student-readiness claim. |
| A free macOS Wails preview package can be created without Apple membership | VERIFIED_IN_SOURCE | `scripts/package-wails-preview.sh` creates an ad-hoc `.app`, `.zip`, `.dmg`, release notice, and checksums without Developer ID credentials | This proves local packaging only; it does not establish Gatekeeper trust, notarization, clean-device acceptance, or public student readiness. |
| Wails exposes an explicit native update check | VERIFIED_BY_UNIT_TEST | `internal/updater/desktop.go` selects only official platform package names; `internal/updater/updater_test.go` covers CLI-only and native-package release metadata | The check uses the stable `/releases/latest` endpoint; the prerelease is deliberately manual and the menu does not silently install or replace the app. |
