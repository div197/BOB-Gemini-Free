...
| The artifact editor can remain empty after an interactive preview has a source, and chat scroll controls can remain hidden after layout | VERIFIED_IN_SOURCE | `internal/server/playground.html` normalizes Marked token objects, recovers an empty editor from the registered source, recalculates directional controls after layout/resize, and `playground_test.go` locks the markers | The in-app browser environment did not load CDN libraries during this run, so full interactive artifact execution still requires a clean browser/device check. |
