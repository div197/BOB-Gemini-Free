...
**Current source candidate:** `v0.2.0-preview.7` from source snapshot `2d42d4443818f594bc8d27f906977b30acce7495` (clean `main` before documentation-only PR #92); locally packaged and signed, not published
...
| Current Preview 7 candidate | `v0.2.0-preview.7` was packaged from source snapshot `2d42d44` (clean `main` before documentation-only PR #92), signed through the local Keychain, and verified locally; it is not a public release | VERIFIED_LOCAL |
...
The clean `main` source snapshot at `2d42d44` was packaged as the explicit next
candidate `v0.2.0-preview.7` after the responsive phone-control and
drawer-boundary change merged. Documentation-only PR #92 later advanced
`main` to `67e5337`; it did not change the candidate bytes. The universal
macOS bundle, signed manifest, ZIP, DMG, release notice, version injection,
local runtime startup, and local updater matrix all passed. The detailed receipt is in
