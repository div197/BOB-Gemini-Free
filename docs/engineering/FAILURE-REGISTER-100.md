...
Latest local evidence now includes public Preview 10 publication and migration
verification on 2026-09-07: the five public macOS assets matched the signed
inputs, the signed discovery feed selected Preview 10, and a preserved Preview
9 bundle copy completed a consented replacement/restart with healthy
`/healthz`. The real 15-check CLI run against the audit gateway on 2026-08-29
also passed 13 checks and failed strict web-route JSON output plus image
generation with HTTP 502. Those failures remain intentionally visible because
...
| 5 | Version, channel, app metadata, Docker metadata, and release assets disagree. | PROTECTED | Release-source checks validate the stable Makefile/Docker version, require one explicit numeric `PREVIEW_VERSION`, and require every preview packager to reject an unset `BOB_RELEASE_VERSION`; unflagged source builds remain `dev`, updater checks accept only canonical stable/`preview.N` identities, and public Preview 10 is reconciled. The operator must advance the candidate after each publication and verify every public asset matrix. |
...
| 8 | An old service worker serves stale HTML, caches API responses, or strands a native Wails frame on the splash screen. | PROTECTED (with native migration) | The browser worker uses versioned local caches, network-first static fetches, and excludes `/v1/` and `/v1beta/`. Native `desktop_shell=1` navigations are `no-store`, bypass the worker, retire legacy registrations before normal PWA registration, and reveal through a validated readiness signal. A same-port native relaunch against a previously worker-controlled origin passed on 2026-09-05; ordinary browser/PWA activation still needs release-device evidence. |
| 9 | Preview/stable channel selection silently downgrades, lets a stable CLI-only release mask a native preview, or hides a failure. | PROTECTED | Bounded channel tests cover stable-first native-package migration, CLI-only stable continuation, preview ordering, and fail-closed stable errors. |
| 10 | Existing pre-Preview-9 devices cannot verify or complete a migration because their updater/key lineage differs, or because the install target is not writable. | EXTERNAL | The current Preview 10 package uses the current project key and the mocked matrix covers legacy Preview 7 and Preview 1–7 selection; the audit Mac proved Preview 9 discovery and a preserved Preview 9 → Preview 10 replacement, but Preview 7 replacement remains unproven, so pilot the same-key transition or perform one manual migration on each relevant lineage before promising a fleet jump. Builds with the unrecoverable legacy key require manual installation. |
...
| 16 | A build is made from uncommitted source or an unrecorded toolchain. | PROTECTED | `verify-release-source.sh` blocks dirty or untracked release packaging, validates the explicit next preview candidate, and rejects preview packagers that guess a version; `record-release-evidence.sh` re-runs source and asset checks and records commit, branch, Go version, host, UTC time, manifest/signature hashes, and asset hashes outside the release tree. |
...
| 24 | Power loss occurs between backup, rename, and helper restart. | PARTIAL | Same-filesystem backup/rollback state is now repaired conservatively at the next native startup: confirmed candidates are finalized, unconfirmed candidates are rolled back, and ambiguous states fail visibly. `b136724` flushes updater metadata before commit and Unix directory entries after swap/recovery transitions; `fd279aa` uses Windows `MoveFileExW` replace-existing/write-through semantics for metadata; fixture tests inject activation-sync failure and cover interrupted states. Recursive app-bundle file durability, Windows directory journaling, and a real OS power-cut run remain external. |
| 25 | A failed update leaves orphaned staging directories or sensitive metadata. | PARTIAL | Committed, matching plans older than 24 hours are removed conservatively when staging begins; plan/confirmation/failure/warning writes use private same-directory atomic replacement, bounded reads, and constrained transaction filenames; startup recovery consumes interrupted plans for the exact install target, while pre-plan crashes remain for manual inspection. |
...
| 30 | A server stalls while sending metadata or an artifact. | PROTECTED | Metadata/artifact clients have total timeouts and bounded readers; read-only release metadata gets at most one cancellation-aware transport/timeout retry, while artifact downloads and HTTP/status failures are not retried. |
...
| 40 | Uploaded DMG, ZIP, manifest, signature, release note, and README describe different bytes or versions. | PARTIAL | The local signer, one-to-one verifier, release receipt, and Preview 3 fresh-directory reconciliation catch the published case; the same public-asset and release-page review remains required for every future release. |
...
| 42 | A cookie expires, is revoked, or loses required tokens during a long session. | PARTIAL | Secure file reload is retained; HTTP 401/403 responses are classified as session/provider authentication failures, invalidate only cached dynamic `/app` tokens, and force the next request through a fresh bootstrap without replaying the rejected request. `internal/gemini/auth_test.go` and `client_test.go` cover refresh, credential preservation, buffered/streaming rejection, and in-flight ordering. Live expiry detection, reauthentication, and provider-session evidence remain open. |
...
| 56 | Gateway API-key auth and Google provider-key routing are confused by the UI or header names, or a client uses a valid case-insensitive Bearer scheme. | PARTIAL | The Studio labels the BOB Gateway Access Key and Google Gemini Developer API key separately, masks both by default, shows a credential map plus a live route-status card, blocks an invalid explicit route before chat/network work, and keeps the dedicated provider header separate. The lower-case Bearer regression and supported-route credential matrix prove the tested boundary; browser/device trust and every future endpoint still require release evidence. |
...
| 71 | Cancelling the leader leaves followers waiting forever, terminates their healthy shared request, or returns a false successful stream. | PROTECTED | A leader's context now detaches only that subscriber; cancellation wins over a concurrently queued final delta, an independent follower remains attached and receives the shared terminal result, and cancellation of the final subscriber cancels the shared runner. Leader-cancellation, leader-deadline, and cancellation-order regressions cover these cases. |
...
| 96 | `FileReader`, PDF/DOCX/XLSX parsing, or OCR reads an oversized local file and freezes the WebView. | PARTIAL | Client-side attachment size is bounded at 32 MiB; extraction output is capped at 1,000,000 characters, PDFs stop at 200 pages, parsing is limited to two concurrent jobs, removed entries are cooperatively ignored, and the obsolete workspace drop/FileReader path was removed so one drop is processed once. Attachment reads now use abortable `FileReader` operations where available, removed entries abort their controller and are removed from the pending parse queue, active PDF.js loading/document tasks are destroyed on cancellation, and Tesseract.js v5 uses a terminable worker when available; the older Promise/legacy OCR fallbacks still only suppress stale results. History attachment metadata is escaped before HTML insertion, and image previews are keyboard/touch controls with an explicit raster-only policy. Real-device CPU/quota evidence and main-thread PDF/DOCX/XLSX profiling are still required before this failure can be called protected. |
| 97 | A malicious webpage uses wildcard CORS/PNA to drive a privileged localhost gateway. | PARTIAL | Default browser access now requires the exact origin of a literal loopback host; an attacker-controlled non-loopback `Host`/DNS alias cannot activate the implicit same-origin shortcut, while remote/alternate origins still require explicit allow-listing. The 2026-08-31 in-app-browser cross-port probe rejected both a models read and a JSON POST/preflight, including an explicit PNA preflight header; public-HTTPS/PNA deployment evidence and the capability-token decision remain. |
| 98 | API keys or sensitive values leak through query strings, referrers, logs, error text, or browser history. | PARTIAL | Provider keys are header-only; the Studio provider toggle fails closed when a key is absent or cleared, and non-loopback provider-key transport requires an explicitly saved HTTPS endpoint; the BOB gateway access key is now withheld from non-loopback cleartext HTTP by one shared browser header guard and the UI reports `HTTPS REQUIRED`; upstream, image, update-check, retry-log, and public Developer API errors now use credential-safe summaries, including a final defense against credential-shaped tokens and URLs; gateway `?key=` authentication is disabled by default and only an explicit legacy opt-in enables it. URL/referrer behavior outside BOB and user-controlled log collectors still require deployment-level controls. |
...
| 100 | Artifact preview, code hydration, scroll controls, external links, clipboard feedback, resize handling, modal lifecycle, deep-refiner failure handling, embedded-engine lifecycle, gateway coexistence, or control semantics diverge across native and hosted builds. | PARTIAL | Source tests cover token normalization, empty-editor recovery, geometry-based scroll controls, one accessible artifact launch action, native button/type semantics, synchronized hidden-drawer state, bounded dialog surfaces, safe delegated attachment controls, escaped persisted attachment icons, raster-only attachment previews, JavaScript-URL-free gateway recovery actions, local/public bundle sync, browser-opening bridge, strict Markdown-link protocol allow-listing, guarded clipboard writes, truthful gateway status after HTTP/stream failures, health-first telemetry that avoids repeated protected-route 401s and clears stale aggregate values, first-committed-status request logging, sandbox isolation including pop-outs, resize hooks, bounded three-stage refiner input/output, strict model resolution, partial-app health/upload/refine guards, optional request logging without a wired logger, nil Gemini-client retry logging, nil mobile-bridge receivers failing closed, nil embedded-engine options being ignored, visible Wails bootstrap errors, exact-version native gateway reuse with stale-version fallback, native update-check dialogs that hide raw transport/metadata details while stating that no app change occurred, native staging/start dialogs that hide filesystem paths and OS error text while providing writable-location recovery guidance, responsive drawer dialog semantics with focus return/Tab trapping/Escape close, the rule that failed refinement leaves the original prompt unchanged, and a 2026-08-31 browser fixture proving split-SSE artifact hydration, non-zero iframe layout, sandbox interaction, non-empty editor recovery, top/bottom scroll controls, and 1440/768/390 geometry. The tablet header overflow was fixed and `/web` synchronized; native, cross-build, CDN, and clean-device checks remain. |
