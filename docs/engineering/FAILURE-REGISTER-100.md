...
| 59 | Typed/multipart content is silently dropped or flattened incorrectly by an adapter. | PROTECTED | Anthropic, OpenAI, direct Developer API, and Google-shaped adapters now fail closed on unsupported blocks/parts, wrong field types, missing text, invalid base64, unknown roles, duplicate declarations, and malformed tool results; provider semantics remain a separate evidence gate. |
