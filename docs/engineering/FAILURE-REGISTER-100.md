...
| 3 | The working branch, `main`, and `origin/main` do not represent the same source. | PROTECTED | Protected PRs #42 and #43 now publish the reviewed runtime/docs state on `origin/main` at `29ab9a1`; the local release-readiness branch may carry a later isolated review commit with an identical tree, so recheck ancestry before each release and never infer state from a local branch name. |
