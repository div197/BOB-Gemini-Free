...
| 3 | The working branch, `main`, and `origin/main` do not represent the same source. | PROTECTED | Protected PR #31 reconciled the earlier hardening, PR #33 merged the release-coherence follow-up, and PR #36 merged the installer trust-anchor follow-up into current `origin/main` at `627e73c`; recheck ancestry before each release and never infer state from a local branch name. |
