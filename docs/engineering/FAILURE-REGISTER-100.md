...
| 3 | The working branch, `main`, and `origin/main` do not represent the same source. | PROTECTED | Protected PR #31 reconciled the reviewed source and published merge commit `c5fa74f`; the local checkout is fast-forwarded to that commit. Recheck ancestry before each release and never infer state from a local branch name. |
...
| 6 | The package embeds a different updater public key from the manifest signer, or a preview target omits the public key at packaging time. | PARTIAL | Preview Make targets now pass the checked-in public key and preview packagers validate channel/version; run the local public/private fingerprint match check before signing and re-verify the embedded public key. |
...
| 73 | Unknown SSE event names or fields crash a client or silently change ordering. | PARTIAL | The Developer API parser ignores standard comments/fields, preserves multi-line `data:` events, rejects empty semantic events as well as empty/`[DONE]`-only/metadata-only streams, and the Web RPC parser rejects streams with no usable text; the raw Google-shaped route validates JSON framing and tracks usable text/inline media/function calls while preserving valid native events. Complete unknown-event diagnostics and Web RPC framing evidence remain open. |
...
| 94 | Image decode, resize, OCR, or base64 expansion exhausts memory. | PARTIAL | Byte, dimension, pixel, and client file limits exist; a vision-upload failure no longer triggers an implicit OCR-only replay that could spend another quota unit or change request semantics. OCR remains user-invoked and browser CPU/concurrent-image budgets still need device evidence. |
...
| 96 | `FileReader`, PDF/DOCX/XLSX parsing, or OCR reads an oversized local file and freezes the WebView. | PARTIAL | Client-side attachment size is bounded at 32 MiB and storage text is capped; worker/cancellation coverage and real-device CPU/quota evidence are still required before this failure can be called protected. |
...
| 98 | API keys or sensitive values leak through query strings, referrers, logs, error text, or browser history. | PARTIAL | Provider keys are header-only; upstream, image, update-check, and retry-log responses now use credential-safe summaries; gateway `?key=` authentication is disabled by default and only an explicit legacy opt-in enables it. URL/referrer behavior outside BOB and user-controlled log collectors still require deployment-level controls. |
| 99 | CDN libraries disappear, CSP/iframe policy changes, or unbounded artifact HTML grows into a browser/desktop denial of service. | PARTIAL | Artifact sandboxing, runtime-error reporting, stable scoped registry IDs, an 8-million-character registry ceiling, and a 2-million-character per-source preview ceiling fail closed before iframe execution. Mermaid and Pyodide iframe loads now report a visible dependency failure instead of silently presenting a broken preview; explicit artifact pop-outs preserve a sandbox, and generated-object URLs are revoked after a grace period. Root-page CDN availability, CSP compatibility, and full offline behavior still require a documented/browser-tested release decision. |
| 100 | Artifact preview, code hydration, scroll controls, external links, resize handling, modal lifecycle, deep-refiner failure handling, or embedded-engine lifecycle diverge across native and hosted builds. | PARTIAL | Source tests cover token normalization, empty-editor recovery, geometry-based scroll controls, local/public bundle sync, browser-opening bridge, sandbox isolation including pop-outs, resize hooks, bounded three-stage refiner input/output, strict model resolution, partial-app health/upload/refine guards, visible Wails bootstrap errors, and the rule that failed refinement leaves the original prompt unchanged; real-browser cross-build, CDN, and clean-device checks remain. |
