...
| 56 | Gateway API-key auth and Google provider-key routing are confused by the UI or header names, or a client uses a valid case-insensitive Bearer scheme. | PARTIAL | The Studio now labels the BOB Gateway Access Key and Google Gemini Developer API key separately, masks both by default, shows a credential map including the web-session/cookie boundary, and keeps the dedicated provider header separate. The lower-case Bearer regression proves standard scheme casing is accepted; a negative matrix for every credential type on every endpoint remains a release test. |
