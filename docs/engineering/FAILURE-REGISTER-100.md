...
| 73 | Unknown SSE event names or fields crash a client or silently change ordering. | PARTIAL | Ignore only documented ignorable events and preserve unknown-event diagnostics without secrets; add fixtures. |
...
| 78 | Nested schemas, enums, arrays, nullable fields, or large arguments are flattened or rejected unexpectedly. | PARTIAL | Golden OpenAI and direct-Developer-API fixtures preserve nested arrays, enums, nullable values, Unicode, and large valid arguments; cross-adapter semantic fidelity and provider acceptance remain open. |
| 79 | `none`, `auto`, `required`, or a named tool choice changes behavior silently. | PARTIAL | Current hints are translated; assert every choice in request/response fixtures and mark unsupported semantics explicitly. |
...
| # | Failure path | Status | Proactive step now |
|---:|---|---|---|
| 81 | OpenAI Responses items unsupported by the translator disappear without a client-visible error. | PARTIAL | Add an unsupported-item error/diagnostic policy and fixtures for every supported item type. |
...
| 88 | Tool-result IDs/names are mismatched, so a continuation is attached to the wrong call. | PROTECTED | `ValidateToolResultReferences` rejects unknown IDs, name mismatches, and ambiguous name-only continuations before prompt flattening or direct translation; valid parallel and invalid-correlation fixtures are covered. |
...
| 95 | The SHA-256 → Scotty reference cache grows forever or retains a ref beyond its session validity. | PARTIAL | The cache now stores only 256 bounded, LRU-like references and includes a hashed single-cookie session scope; configured multi-account pools disable reuse. Scotty reference TTL/expiry and concurrent upload single-flight remain open. |
...
| 98 | API keys or sensitive values leak through query strings, referrers, logs, error text, or browser history. | PARTIAL | Provider keys are header-only and errors redact them; remove/forbid query-key compatibility where possible and add log/referrer tests. |
