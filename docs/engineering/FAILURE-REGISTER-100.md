...
| 3 | The working branch, `main`, and `origin/main` do not represent the same source. | PROTECTED | Protected PR #31 reconciled the earlier hardening, and PR #33 merged the release-coherence follow-up into current `origin/main` at `2f5d498`; recheck ancestry before each release and never infer state from a local branch name. |
