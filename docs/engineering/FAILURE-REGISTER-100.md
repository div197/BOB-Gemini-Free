...
| 5 | Version, channel, app metadata, Docker metadata, and release assets disagree. | PROTECTED | Release-source checks validate the stable Makefile/Docker version, require one explicit numeric `PREVIEW_VERSION`, and require every preview packager to reject an unset `BOB_RELEASE_VERSION`; unflagged source builds remain `dev`, updater checks accept only canonical stable/`preview.N` identities, and Preview 6 is reconciled. The operator must advance the candidate after each publication and verify every public asset matrix. |
...
| 10 | Existing pre-Preview-6 devices cannot verify or complete a migration because their updater/key lineage differs, or because the install target is not writable. | EXTERNAL | The current Preview 6 package uses the current project key and the mocked matrix covers legacy Preview 7 and Preview 1–5 selection; still pilot the documented same-key bridge or perform one manual migration on each relevant lineage before promising a fleet jump. Builds with the unrecoverable legacy key require manual installation. |
