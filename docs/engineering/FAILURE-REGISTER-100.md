...
| 3 | The working branch, `main`, and `origin/main` do not represent the same source. | PROTECTED | Protected PRs #31, #33, #36, #38, and #40 merged the reviewed hardening into current `origin/main` at `a80f08d`; recheck ancestry before each release and never infer state from a local branch name. |
...
| 96 | `FileReader`, PDF/DOCX/XLSX parsing, or OCR reads an oversized local file and freezes the WebView. | PARTIAL | Client-side attachment size is bounded at 32 MiB; extraction output is capped at 1,000,000 characters, PDFs stop at 200 pages, parsing is limited to two concurrent jobs, and removed entries are cooperatively ignored; worker termination and real-device CPU/quota evidence are still required before this failure can be called protected. |
