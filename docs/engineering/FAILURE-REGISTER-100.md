...
| 96 | `FileReader`, PDF/DOCX/XLSX parsing, or OCR reads an oversized local file and freezes the WebView. | PARTIAL | Client-side attachment size is bounded at 32 MiB; extraction output is capped at 1,000,000 characters, PDFs stop at 200 pages, parsing is limited to two concurrent jobs, removed entries are cooperatively ignored, and the obsolete workspace drop/FileReader path was removed so one drop is processed once; worker termination and real-device CPU/quota evidence are still required before this failure can be called protected. |
...
| 98 | API keys or sensitive values leak through query strings, referrers, logs, error text, or browser history. | PARTIAL | Provider keys are header-only; upstream, image, update-check, retry-log, and public Developer API errors now use credential-safe summaries, including a final defense against credential-shaped tokens and URLs; gateway `?key=` authentication is disabled by default and only an explicit legacy opt-in enables it. URL/referrer behavior outside BOB and user-controlled log collectors still require deployment-level controls. |
...
| 100 | Artifact preview, code hydration, scroll controls, external links, clipboard feedback, resize handling, modal lifecycle, deep-refiner failure handling, or embedded-engine lifecycle diverge across native and hosted builds. | PARTIAL | Source tests cover token normalization, empty-editor recovery, geometry-based scroll controls, local/public bundle sync, browser-opening bridge, guarded clipboard writes, sandbox isolation including pop-outs, resize hooks, bounded three-stage refiner input/output, strict model resolution, partial-app health/upload/refine guards, optional request logging without a wired logger, nil mobile-bridge receivers failing closed, nil embedded-engine options being ignored, visible Wails bootstrap errors, and the rule that failed refinement leaves the original prompt unchanged; real-browser cross-build, CDN, and clean-device checks remain. |
