...
| 98 | API keys or sensitive values leak through query strings, referrers, logs, error text, or browser history. | PARTIAL | Provider keys are header-only; the Studio provider toggle fails closed when a key is absent or cleared; upstream, image, update-check, retry-log, and public Developer API errors now use credential-safe summaries, including a final defense against credential-shaped tokens and URLs; gateway `?key=` authentication is disabled by default and only an explicit legacy opt-in enables it. URL/referrer behavior outside BOB and user-controlled log collectors still require deployment-level controls. |
