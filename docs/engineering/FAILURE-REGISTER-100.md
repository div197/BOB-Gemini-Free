...
**Audit date:** 2026-08-30 (Asia/Kolkata)
...
| 3 | The working branch, `main`, and `origin/main` do not represent the same source. | PROTECTED | Protected PR #31 reconciled the earlier hardening, PR #33 merged the release-coherence follow-up, PR #36 merged the installer trust-anchor follow-up, and PR #38 merged the session-only gateway-auth follow-up into current `origin/main` at `f3a0a8c`; recheck ancestry before each release and never infer state from a local branch name. |
