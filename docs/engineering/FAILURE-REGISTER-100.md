...
| 5 | Version, channel, app metadata, Docker metadata, and release assets disagree. | PROTECTED | Release-source checks validate the stable Makefile/Docker version and all preview packager defaults; unflagged source builds remain `dev`, updater checks accept only canonical stable/`preview.N` identities, and Preview 3 is reconciled. Every future public asset matrix still requires operator verification. |
