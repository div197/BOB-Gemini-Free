...
| 96 | `FileReader`, PDF/DOCX/XLSX parsing, or OCR reads an oversized local file and freezes the WebView. | PARTIAL | Client-side attachment size is bounded at 32 MiB; extraction output is capped at 1,000,000 characters, PDFs stop at 200 pages, parsing is limited to two concurrent jobs, removed entries are cooperatively ignored, and the obsolete workspace drop/FileReader path was removed so one drop is processed once. History attachment metadata is escaped before HTML insertion, and image previews are keyboard/touch controls with an explicit raster-only policy. Worker termination and real-device CPU/quota evidence are still required before this failure can be called protected. |
