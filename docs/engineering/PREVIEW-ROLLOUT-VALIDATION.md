...
**Current public target:** controlled macOS preview `v0.2.0-preview.10`, for
...
historical; Preview 10 is the current signed public prerelease, and the earlier
...
can then use this updater. This is separate from the current `v0.2.0-preview.10`
...
same-key Preview 10 through its preview-only path, subject to the live device
...
- for the existing public `v0.1.7-preview.7` path, check the published
  same-key Preview 10 (or the published Preview 1 bridge only if that
...
`v0.2.0-preview.10` are public. Preview 10's exact public macOS assets have been re-downloaded,
signature-verified, and byte-reconciled with the local Keychain-backed
publication input. A preserved Preview 9 bundle completed the real consented
Preview 9 → Preview 10 replacement and healthy restart path. A Preview 5 or
Preview 7 → Preview 10 installed transition therefore remains a device gate.
The remaining
rollout gates are:
...
3. **COMPLETED for one writable bundle copy:** the Preview 9 → Preview 10
   updater artifacts were signed and manually verified, and the copy completed
   replacement/restart; deliberate rollback, clean-device recovery, and the
   Preview 7 installed-base transition remain open;
