...
good_func = """func (c *CookieCache) GetAtToken(ctx context.Context, httpClient Requester, authUser string) string {
	...
	// Use static agent for this metadata fetch, no need for TLS stealth on this specific call
	req.Header.Set("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36")
...
# Actually, the file has req.Header.Set("Cookie", cookie) because I injected it incorrectly before. 
# Let's just regex replace the whole function properly.
# The previous state in the file currently has `cookie`.
import re
