...
| **Native updater status** | The immutable `v0.2.0-preview.1` bridge, historical Preview 3, controlled macOS `v0.2.0-preview.4`, and current macOS universal `v0.2.0-preview.5` are public. Preview 5 is signed with the checked-in project key, was re-downloaded and byte-reconciled, and successfully migrated one writable `/Applications` Preview 1 installation on the audit Mac. Current previews add explicit Preview → Stable migration. Stable `v0.2.0` remains gated on rollback, clean-device, pilot, and platform-trust acceptance. Older builds with an unrecoverable key still require one manual migration |
...
* The latest stable GitHub release contains CLI binaries. The public [v0.2.0-preview.5 controlled macOS preview](https://github.com/div197/BOB-Gemini-Free/releases/tag/v0.2.0-preview.5) is the current downloadable universal preview; Preview 4 remains immutable historical input. Public `main` contains the post-Preview-4 artifact-preview, responsive-header, multiline-SSE, credential-boundary, browser-boundary, and credential-input fixes at `ade691d`; the immutable Preview 5 package remains frozen at its signed `c28d787` source baseline and does not contain the post-publication PR #77/#78 follow-ups. Preview 5 passed local package/startup checks, public-byte reconciliation, and one real writable `/Applications` migration from Preview 1. The immutable [v0.2.0-preview.1 migration bridge](https://github.com/div197/BOB-Gemini-Free/releases/tag/v0.2.0-preview.1), historical Preview 3, and superseded Preview 2 remain available as historical release inputs. It is an authentic open-source beta, platform trust is not yet established, and Windows/Linux remain separate preview targets. Stable `v0.2.0` is not yet published; rollback, clean-device, pilot, and provider gates remain open. See [`RELEASE-AUDIT-2026-08-31.md`](docs/engineering/RELEASE-AUDIT-2026-08-31.md), [`RELEASE-TRANSITION-AUDIT-2026-08-31.md`](docs/engineering/RELEASE-TRANSITION-AUDIT-2026-08-31.md), and the [`Preview 5 release notes`](docs/engineering/PREVIEW-5-RELEASE-NOTES.md).
