...
| **Native updater status** | Public macOS universal [`v0.2.0-preview.10`](https://github.com/div197/BOB-Gemini-Free/releases/tag/v0.2.0-preview.10) is the current controlled beta, built from merged `main` commit `1862d31`, signed through the owner-controlled macOS Keychain, and reconciled across all five public assets. The installed Preview 9 on the audit Mac discovered Preview 10, and a preserved Preview 9 bundle copy completed the real consented replacement/restart path. Current-source builds use a detached-signed discovery feed before the fixed GitHub API fallback; this reduces API bursts but does not change the consent or signed-package boundary. Preview 7 and current-key v0.2 previews can continue through same-key signed previews; obsolete/missing-key builds still require one manual migration. Stable `v0.2.0` remains gated on rollback, clean-device, pilot, provider, and platform-trust acceptance. See [`PREVIEW-10-PUBLIC-VERIFICATION-2026-09-07.md`](docs/engineering/PREVIEW-10-PUBLIC-VERIFICATION-2026-09-07.md). |
...
* The current source has a build-pinned update policy. Newly built previews can
  ...
  current public `v0.2.0-preview.10` package.
...
on the [public Preview 10 release page](https://github.com/div197/BOB-Gemini-Free/releases/tag/v0.2.0-preview.10). Do not use a locally built or unpublished candidate for student installation.
...
`v0.2.0-preview.10` carries that key; it still requires explicit user consent
