...
| **Native updater status** | The immutable `v0.2.0-preview.1` bridge, historical Preview 3, and controlled macOS `v0.2.0-preview.4` are public. Public `main` now includes the route-clarity and credential-boundary fixes at merge commit `a68eb39`; a fresh signed `v0.2.0-preview.5` candidate from that exact source passed local package/startup smoke checks but is not public until public-byte and installed-update gates pass. Current previews add explicit Preview → Stable migration. Stable `v0.2.0` remains gated on clean-device and pilot acceptance. Older builds with an unrecoverable key still require one manual migration |
