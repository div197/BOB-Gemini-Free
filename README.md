...
| **Native updater status** | The immutable `v0.2.0-preview.1` bridge, historical Preview 3–5, and current macOS universal `v0.2.0-preview.6` are public. Preview 6 is signed with the checked-in project key, was re-downloaded and byte-reconciled, and its package passed local coexistence/health smoke. The earlier Preview 1 → Preview 5 installed migration remains the only live migration observation. Current previews add explicit Preview → Stable migration. Stable `v0.2.0` remains gated on rollback, clean-device, pilot, and platform-trust acceptance. Older builds with an unrecoverable key still require one manual migration |
...
* The latest stable GitHub release contains CLI binaries. The public [v0.2.0-preview.6 controlled macOS preview](https://github.com/div197/BOB-Gemini-Free/releases/tag/v0.2.0-preview.6) is now the current downloadable universal preview, built from source target `f9b3410`; Preview 5 and Preview 4 remain immutable historical inputs. Public `main` contains the post-Preview-5 settings, telemetry, release-version, version-aware desktop coexistence, release-state reconciliation, and secure gateway-key transport guard; the immutable Preview 6 package carries those reviewed follow-ups. The package was signed, verified, coexistence-smoke-tested locally, published manually, and re-downloaded for exact byte verification. The immutable [v0.2.0-preview.1 migration bridge](https://github.com/div197/BOB-Gemini-Free/releases/tag/v0.2.0-preview.1), historical Preview 3, and superseded Preview 2 remain available as historical release inputs. It is an authentic open-source beta, platform trust is not yet established, and Windows/Linux remain separate preview targets. Stable `v0.2.0` is not yet published; rollback, clean-device, pilot, and provider gates remain open. See [`RELEASE-AUDIT-2026-08-31.md`](docs/engineering/RELEASE-AUDIT-2026-08-31.md), [`PREVIEW-6-LOCAL-VERIFICATION-2026-08-31.md`](docs/engineering/PREVIEW-6-LOCAL-VERIFICATION-2026-08-31.md), and the [`Preview 6 release notes`](docs/engineering/PREVIEW-6-RELEASE-NOTES.md).
