...
| **Native updater status** | The immutable `v0.2.0-preview.1` bridge, historical Preview 3, and controlled macOS `v0.2.0-preview.4` are public; Preview 4 is the current same-key preview candidate for controlled evaluation. Public `main` is now ahead of the immutable Preview 4 assets with artifact-preview, responsive-header, and multiline-SSE fixes, and the next package candidate is explicitly `v0.2.0-preview.5` (not published). Current previews add explicit Preview → Stable migration. Stable `v0.2.0` remains gated on clean-device and pilot acceptance. Older builds with an unrecoverable key still require one manual migration |
...
* The latest stable GitHub release contains CLI binaries. The public [v0.2.0-preview.4 controlled macOS preview](https://github.com/div197/BOB-Gemini-Free/releases/tag/v0.2.0-preview.4) is the current universal preview and the latest same-key preview candidate for controlled evaluation. It contains the corrected branded ZIP/DMG package, signed project update manifest, native maximize behavior, default-browser link routing, expanded English/Hindi studio UI, bounded provider retries, visible failure handling, responsive drawer focus protection, and stale-session recovery after explicit upstream authentication rejection. Public `main` now also contains the post-Preview-4 artifact-preview, responsive-header, and multiline-SSE fixes; those immutable Preview 4 assets are unchanged, and the next package candidate is `v0.2.0-preview.5`. The immutable [v0.2.0-preview.1 migration bridge](https://github.com/div197/BOB-Gemini-Free/releases/tag/v0.2.0-preview.1), historical Preview 3, and superseded Preview 2 remain available as historical release inputs. It is an authentic open-source beta, platform trust is not yet established, and Windows/Linux remain separate preview targets. Stable `v0.2.0` is not yet published; see [`RELEASE-READINESS-v0.2.0.md`](docs/engineering/RELEASE-READINESS-v0.2.0.md), [`RELEASE-TRANSITION-AUDIT-2026-08-31.md`](docs/engineering/RELEASE-TRANSITION-AUDIT-2026-08-31.md), and the [`Preview 4 release notes`](docs/engineering/PREVIEW-4-RELEASE-NOTES.md).
...
* The current source has a build-pinned update policy. Newly built previews can
  ...
  manual migration because it predates the embedded trust key. Preview 6 also
  needs one manual migration to Preview 7 because the original Preview 6
  signing key was not recoverable.
...
`v0.2.0-preview.4` carries that key; it still requires explicit user consent
